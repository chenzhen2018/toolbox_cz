...
from six.moves import xrange    # python 3删除了xrange
...
def _process_image(filename, coder):
    ...
    """
    # Read the image file.
    with open(filename, 'rb') as f: # 由r改成rb
        image_data = f.read()
...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames,
    ...
    num_threads = len(ranges)   # 2, ranges: [[0, 600], [600, 1200]]
    assert not num_shards % num_threads
    num_shards_per_batch = int(num_shards / num_threads)    # 1

    shard_ranges = np.linspace(ranges[thread_index][0],
                               ranges[thread_index][1],
                               num_shards_per_batch + 1).astype(int)
    num_files_in_thread = ranges[thread_index][1] - ranges[thread_index][0] # 600-0 或者 1200-600
...
def _process_image_files(name, filenames, texts, labels, num_shards, command_args):
    ...
    spacing = np.linspace(0, len(filenames), command_args.num_threads + 1).astype(np.int)   # [0, 600, 1200]，产生的样本数量；其中commanc_args.num_threads: 2
    ranges = []
    for i in xrange(len(spacing) - 1):
        ranges.append([spacing[i], spacing[i + 1]]) # [[0, 600], [600, 1200]]
    ...
    coord = tf.train.Coordinator()  # 创建一个监视所有线程何时完成的机制。

    # Create a generic TensorFlow-based utility for converting all image codings.
    coder = ImageCoder()    # 用于转换图像编码
...
def _find_image_files(data_dir, labels_file, command_args):
    ...
    unique_labels = [l.strip() for l in tf.gfile.FastGFile(
        labels_file, 'r').readlines()]  # 返回的是一个列表，其中每一个元素是一个字符串，对应着label_file的一行；
    ...
    label_index = command_args.class_label_base # label从0开始

    # Construct the list of JPEG files and labels.
    for text in unique_labels:
        jpeg_file_path = '%s/%s/*' % (data_dir, text)   # data_dir/text/* 路径下的所有文件
        matching_files = tf.gfile.Glob(jpeg_file_path)  # 返回给定模式匹配的文件名的字符串列表

        labels.extend([label_index] * len(matching_files))  # 将每一张图片的label迭代的加入进label这个列表中
        texts.extend([text] * len(matching_files))  # 添加相应的类别字符串，与labels相对应；texts是字符串类别，labels是数值型；
        filenames.extend(matching_files)    # 文件名列表

        if not label_index % 100:
            logging.info('Finished finding files in %d of %d classes.' % (
                label_index, len(labels)))
        label_index += 1    # 循环依次是每个类别文件夹，label_index相应的加1

    # for循环结束后，会得到labels, texts, filenames三个列表；

    # Shuffle the ordering of all image files in order to guarantee
    # random ordering of the images with respect to label in the
    # saved TFRecord files. Make the randomization repeatable.
    shuffled_index = list(range(len(filenames)))    # 修改过，加了list
    random.seed(12345)
    random.shuffle(shuffled_index)  # 得到一个随机排列的列表，范围在：0 ~ len(filename)-1

    # 根据得到的随机列表，对labels, texts, filenames三个列表乱序；
